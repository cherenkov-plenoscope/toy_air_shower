...
ELECTRON_REST_ENERGY = ELECTRON_MASS * SPEED_OF_LIGHT ** 2.0
...
PERMABILITY_AIR = 4.0 * np.pi * 1e-7 * 1.00000037  # H/m
...
SPLITING_DEPTH_ELECTRON = RADIATION_LENGTH_AIR_ELECTRON * np.log(2.0)  # g/cm^2
# Depth to be traversed before Energy is reduced to 1/2.
SPLITTING_DEPTH_GAMMA = RADIATION_LENGTH_AIR_GAMMA * np.log(2.0)  # g/cm^2

BREMSSTRAHLUNGS_RATE_PER_UNIT_DEPTH_ELECTRON = (
    1.0 / SPLITING_DEPTH_ELECTRON
)  # cm^2/g
PAIR_PROD_RATE_PER_UNIT_DEPTH_GAMMA = 1.0 / SPLITTING_DEPTH_GAMMA  # cm^2/g
...
def dE_over_dz(q, beta, n, mu, wavelength_start, wavelength_end):
    ...
    omega_start = 2.0 * np.pi * SPEED_OF_LIGHT / wavelength_start
    omega_end = 2.0 * np.pi * SPEED_OF_LIGHT / wavelength_end
    return (
        q ** 2.0
        / (4.0 * np.pi)
        * mu
        * (1.0 - 1.0 / (beta ** 2.0 * n ** 2.0))
        * 0.5
        * (omega_end ** 2.0 - omega_start ** 2.0)
    )
...
def natural_velocity(kinetic_energy, rest_energy):
    gamma = lorentz_factor(
        kinetic_energy=kinetic_energy, rest_energy=rest_energy
    )
    return np.sqrt(1.0 - 1.0 / gamma ** 2)


def make_cherenkov_spectrum(wvl_range):
    spectrum = 1.0 / wvl_range ** 2
...
def gamma_ray_first_interaction(primary_energy):
    ...
    particles = [
        ...
        {
            "type": "electron",
            "start_energy": primary_energy / 2.0,
            "energy": primary_energy / 2.0,
...
def draw_wavelength(cherenkov_spectrum_cdf, wavelength_range):
    # CDF is cummulative_distribution_function
    wavelength = np.interp(
        x=np.random.uniform(), xp=cherenkov_spectrum_cdf, fp=wavelength_range
    )
    return wavelength
...
def simulate_gamma_ray_air_shower(
    ...
    particles, todo = gamma_ray_first_interaction(
    ...
    (
        energy_fraction_electron,
        energy_fraction_gamma_ray,
    ) = heitler_correction_spliting(bremsstrahlung_correction_factor)
    ...
    while True:
        ...
        if particles[todo[0]]["type"] == "gamma":
            ...
            current_depth = altitude_to_depth(
                particles[todo[0]]["start_altitude"]
            )
            next_interaction_depth = (
                current_depth + depth_until_next_interaction
            )
            next_interaction_altitude = depth_to_altitude(
                next_interaction_depth
            )
            assert (
                next_interaction_altitude
                < particles[todo[0]]["start_altitude"]
            )
            ...
            if particles[todo[0]]["energy"] < CRITICAL_ENERGY:
                ...
                # pair-production
                next_gen_energy = 0.5 * particles[todo[0]]["energy"]
            ...
            if particles[todo[0]]["energy"] < CRITICAL_ENERGY:
                ...
                particles.append(
                    {
                        "type": "electron",
                        "start_energy": energy_fraction_electron
                        * particles[todo[0]]["energy"],
                        "energy": energy_fraction_electron
                        * particles[todo[0]]["energy"],
                        "start_altitude": next_interaction_altitude,
                        "mother": todo[0],
                    }
                ...
                particles.append(
                    {
                        "type": "gamma",
                        "start_energy": energy_fraction_gamma_ray
                        * particles[todo[0]]["energy"],
                        "energy": energy_fraction_gamma_ray
                        * particles[todo[0]]["energy"],
                        "start_altitude": next_interaction_altitude,
                        "mother": todo[0],
                    }
